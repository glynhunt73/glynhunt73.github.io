...
class Snake {
	...
	update() {
		...
		if (this.game.checkCollision(this, this.game.food)) {
			let color
			if (this.game.food.frameY === 1) { //poisonous food
				this.score--
				color = 'black'
				...
				this.length++
				color = 'gold'
				this.game.sound.play(this.game.sound.biteSound[Math.floor(Math.random() * this.game.sound.biteSound.length)])
	...
	draw() {
		this.segments.forEach((segment, i) => {
			if (this.game.debug) {
				if (i === 0) this.game.c.fillStyle = 'brown'
				else this.game.c.fillStyle = this.color
				this.game.c.fillRect(
			...
			this.game.c.drawImage(
				...
				segment.frameY * this.spriteHieght, 
				this.spriteWidth, 
				this.spriteHieght,  //sx, sy, sw, sh,
				segment.x * this.game.cellSize, 
				segment.y * this.game.cellSize, 
				this.width, 
				this.height
	...
	setSpriteFrame(index) {
		...
		if (index === 0) { // head
			if (segment.y < nextSegment.y) { // up
				if (this.game.food.y === segment.y -1 &&
				...
				if (this.game.food.x === segment.x -1 &&
...
class ComputerAI extends Snake {
	...
	update() {
		super.update()
		if (this.x === this.game.food.x && this.speedY === 0 ||
